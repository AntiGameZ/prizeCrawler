...
@Service("T01009")
public class QiXingCai extends AbstractLottype{

	private static Logger logger = LoggerFactory.getLogger(QiXingCai.class);
	private static final long WAITTIME = Long.parseLong(ResourceBundle.getBundle("waittime").getString("waittime.T01009"));
	...
	{
		map.put("I10001", new QXC500wan());
		map.put("I10002", new QXCOkooo());
		map.put("I10003", new QXCTaobao());
	}
	...
	public void crawl(AgencyPrizeCode agencyPrizeCode) {
		...
		try {
			...
			PrizeInfo prizeInfo = iagency.crawlFromAgency("T01009", agencyPrizeCode.getId().getBatchcode());
			
			if(prizeInfo.getWinbasecode()!=null&&prizeInfo.getWinbasecode().trim().length()==7) {
	...
	public boolean isCheck() {
		return lotCheckSwitchService.isCheck("T01009");
	}
