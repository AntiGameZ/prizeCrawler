...
@Service("T01002")
public class PaiLieSan extends AbstractLottype{

	private static Logger logger = LoggerFactory.getLogger(PaiLieSan.class);
	private static final long WAITTIME = Long.parseLong(ResourceBundle.getBundle("waittime").getString("waittime.T01002"));
	...
	{
		map.put("I10001", new PLS500wan());
		map.put("I10002", new PLSOkooo());
		map.put("I10003", new PLSTaobao());
	}
	...
	public void crawl(AgencyPrizeCode agencyPrizeCode) {
		...
		try {
			...
			PrizeInfo prizeInfo = iagency.crawlFromAgency("T01002", agencyPrizeCode.getId().getBatchcode());
			
			if(prizeInfo.getWinbasecode()!=null&&prizeInfo.getWinbasecode().trim().length()==3) {
	...
	public boolean isCheck() {
		return lotCheckSwitchService.isCheck("T01002");
	}
