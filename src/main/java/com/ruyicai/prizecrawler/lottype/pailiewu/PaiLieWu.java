...
@Service("T01011")
public class PaiLieWu extends AbstractLottype{

	private static Logger logger = LoggerFactory.getLogger(PaiLieWu.class);
	private static final long WAITTIME = Long.parseLong(ResourceBundle.getBundle("waittime").getString("waittime.T01011"));
	...
	{
		map.put("I10001", new PLW500wan());
		map.put("I10002", new PLWOkooo());
		map.put("I10003", new PLWTaobao());
	}
	...
	public void crawl(AgencyPrizeCode agencyPrizeCode) {
		...
		try {
			...
			PrizeInfo prizeInfo = iagency.crawlFromAgency("T01011", agencyPrizeCode.getId().getBatchcode());
			
			if(prizeInfo.getWinbasecode()!=null&&prizeInfo.getWinbasecode().trim().length()==5) {
	...
	public boolean isCheck() {
		return lotCheckSwitchService.isCheck("T01011");
	}
