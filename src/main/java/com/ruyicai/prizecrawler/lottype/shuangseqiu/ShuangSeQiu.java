...
@Service("F47104")
public class ShuangSeQiu extends AbstractLottype{

	private static Logger logger = LoggerFactory.getLogger(ShuangSeQiu.class);
	private static final long WAITTIME = Long.parseLong(ResourceBundle.getBundle("waittime").getString("waittime.F47104"));
	...
	{
		map.put("I10001", new SSQ500wan());
		map.put("I10002", new SSQOkooo());
		map.put("I10003", new SSQTaobao());
	}
	...
	public void crawl(AgencyPrizeCode agencyPrizeCode) {
		...
		try {
			...
			PrizeInfo prizeInfo = iagency.crawlFromAgency("F47104", agencyPrizeCode.getId().getBatchcode());
			
			if(prizeInfo.getWinbasecode()!=null&&prizeInfo.getWinbasecode().trim().length()==12
	...
	public boolean isCheck() {
		return lotCheckSwitchService.isCheck("F47104");
	}
