...
@Service("F47103")
public class ThreeD extends AbstractLottype{

	private static Logger logger = LoggerFactory.getLogger(ThreeD.class);
	
	private static final long WAITTIME = Long.parseLong(ResourceBundle.getBundle("waittime").getString("waittime.F47103"));
	...
	{
		map.put("I10001", new D3500wan());
		map.put("I10002", new D3Okooo());
		map.put("I10003", new D3Taobao());
	}
	...
	public void crawl(AgencyPrizeCode agencyPrizeCode) {
		...
		try {
			...
			PrizeInfo prizeInfo = iagency.crawlFromAgency("F47103", agencyPrizeCode.getId().getBatchcode());
			
			if(prizeInfo.getWinbasecode()!=null&&prizeInfo.getWinbasecode().trim().length()==6) {
	...
	public boolean isCheck() {
		return lotCheckSwitchService.isCheck("F47103");
	}
